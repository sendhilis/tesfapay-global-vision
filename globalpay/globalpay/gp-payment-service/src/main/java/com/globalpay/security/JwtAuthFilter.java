...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        } catch (Exception ex) { log.warn("JWT filter: {}", ex.getMessage()); }
        chain.doFilter(request, response);
    }

    private Claims parse(String token) {
        byte[] key = jwtSecret.getBytes(); byte[] k32 = new byte[32];
        System.arraycopy(key, 0, k32, 0, Math.min(key.length, 32));
        return Jwts.parser().verifyWith(Keys.hmacShaKeyFor(k32)).build()

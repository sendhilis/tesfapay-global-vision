...
const AgencyCashIn = () => {
  ...
  const handleFindCustomer = async () => {
    ...
    if (!profile) {
      // Fallback to mock customers for demo
  ...
  const handleOtpVerify = async () => {
    ...
    if (customerAccountId) {
      await supabase.from('accounts').update({
        balance: customerBalance + amt,
        available_balance: customerBalance + amt,
        ...
        <div className="flex items-center gap-1 mb-4">
          {['find-customer', 'enter-amount', 'confirm', 'otp-verify', 'success'].map((s, i) => (
            <div key={s} className={`h-1.5 flex-1 rounded-full transition-colors ${
              ['find-customer', 'enter-amount', 'confirm', 'otp-verify', 'success'].indexOf(step) >= i ? 'bg-success' : 'bg-muted'
            }`} />
          ))}
        </div>
            ...
            <motion.div key="find" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Enter customer phone number</p>
                </div>
              </div>
              ...
              <Button onClick={handleFindCustomer} disabled={loading} className="w-full bg-success hover:bg-success/90">
          ...
          {step === 'enter-amount' && (
            <motion.div key="amount" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <div>
                  <h2 className="text-lg font-bold text-foreground">Cash In for {customerName}</h2>
                  <p className="text-xs text-muted-foreground">Current balance: {fmt(customerBalance)} ETB</p>
                </div>
              </div>
              <div className="bg-card rounded-xl border border-border p-4 mb-4">
                <label className="text-xs font-medium text-muted-foreground mb-2 block">Deposit Amount</label>
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={handleConfirm} className="flex-1 bg-success hover:bg-success/90">Continue</Button>
              </div>
            </motion.div>
          )}
          ...
          {step === 'confirm' && (
            <motion.div key="confirm" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <div>
                  <h2 className="text-lg font-bold text-foreground">Confirm Cash-In</h2>
              ...
              <div className="bg-card rounded-xl border border-border p-4 space-y-3 mb-4">
                ...
                <div className="flex justify-between text-sm"><span className="text-muted-foreground">Phone</span><span className="font-medium">{customerPhone}</span></div>
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={() => setStep('otp-verify')} className="flex-1 bg-success hover:bg-success/90">Proceed</Button>
              </div>
            </motion.div>
          )}
          ...
          {step === 'otp-verify' && (
            <motion.div key="otp" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div className="bg-card rounded-xl border border-border p-6 text-center mb-4">
                ...
                <p className="text-[11px] text-muted-foreground mt-3">Enter any 6 digits for demo</p>
              </div>
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => setStep('confirm')} className="flex-1">{t('common.back')}</Button>
                <Button onClick={handleOtpVerify} disabled={loading || otp.length < 6} className="flex-1 bg-success hover:bg-success/90">
          ...
          {step === 'success' && (
            <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
              <div className="text-center py-6">
                ...
                <h2 className="text-xl font-bold text-foreground mb-1">Cash-In Successful!</h2>
                <p className="text-sm text-muted-foreground">{fmt(parseFloat(amount))} ETB deposited for {customerName}</p>
